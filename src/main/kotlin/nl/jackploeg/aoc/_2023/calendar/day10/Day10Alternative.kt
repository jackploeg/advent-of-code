...
import nl.jackploeg.aoc._2023.calendar.day10.Day10Alternative.DirectionCameFrom.*
import nl.jackploeg.aoc._2023.calendar.day10.Day10Alternative.LandType.*
import nl.jackploeg.aoc.generators.InputGenerator.InputGeneratorFactory
import java.util.*
import javax.inject.Inject

class Day10Alternative @Inject constructor(
  ...
  private fun findOurPipes(
    ...
    when (area[startLocationRow][startLocationCol]) {
      ...
      NE_PIPE -> { stepsToTake.add(travelNorth); stepsToTake.add(travelEast)  }
      NW_PIPE -> { stepsToTake.add(travelNorth); stepsToTake.add(travelWest)  }
      SW_PIPE -> { stepsToTake.add(travelWest);  stepsToTake.add(travelSouth) }
      SE_PIPE -> { stepsToTake.add(travelSouth); stepsToTake.add(travelEast)  }
